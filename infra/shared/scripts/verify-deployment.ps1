...
param(
    ...
    [int]$SshPort = 65002,
    [string]$ProdPath = "/home/u909075950/domains/hireaccord.com/public_html"
)

$ErrorActionPreference = "Stop"

Write-Host "ðŸ” Deployment Verification" -ForegroundColor Cyan
...
Write-Host "ðŸ“‹ Getting changed files from commit..." -ForegroundColor Yellow
$changedFiles = git show $Commit --name-only --format="" | Where-Object { 
    $_ -like "wp-content/*" -and $_ -like "*.php" 
}

if (-not $changedFiles) {
    Write-Host "âœ… No wp-content/ PHP files changed in commit $Commit" -ForegroundColor Green
    exit 0
}
...
foreach ($file in $changedFiles) {
    ...
    Write-Host "ðŸ”Ž Checking: $file" -ForegroundColor Cyan
    
    # Check if local file exists
    if (-not (Test-Path $localPath)) {
        Write-Host "  âš ï¸  Local file not found (deleted?)" -ForegroundColor Yellow
        continue
    }
    
    # Get local file size
    $localSize = (Get-Item $localPath).Length
    Write-Host "  ðŸ“ Local: $localSize bytes" -ForegroundColor Gray
    ...
    if ($prodStat -eq "MISSING") {
        Write-Host "  âŒ Production: FILE MISSING" -ForegroundColor Red
        $missing += $file
        continue
    }
    
    $prodSize = [int]$prodStat
    Write-Host "  ðŸ“ Production: $prodSize bytes" -ForegroundColor Gray
    
    # Compare sizes
    if ($localSize -eq $prodSize) {
        Write-Host "  âœ… MATCH" -ForegroundColor Green
        $verified += $file
    } else {
        Write-Host "  âŒ SIZE MISMATCH" -ForegroundColor Red
        $mismatches += @{
            File = $file
            LocalSize = $localSize
            ProdSize = $prodSize
...
Write-Host ""
Write-Host "âœ… Verified: $($verified.Count)" -ForegroundColor Green
Write-Host "âŒ Mismatches: $($mismatches.Count)" -ForegroundColor $(if ($mismatches.Count -gt 0) { "Red" } else { "Gray" })
Write-Host "ðŸš« Missing: $($missing.Count)" -ForegroundColor $(if ($missing.Count -gt 0) { "Red" } else { "Gray" })
Write-Host ""

# Report issues
if ($mismatches.Count -gt 0) {
    Write-Host "âŒ SIZE MISMATCHES DETECTED:" -ForegroundColor Red
    foreach ($m in $mismatches) {
        Write-Host "  â€¢ $($m.File)" -ForegroundColor Yellow
...
if ($missing.Count -gt 0) {
    Write-Host "ðŸš« MISSING FILES IN PRODUCTION:" -ForegroundColor Red
    foreach ($f in $missing) {
        Write-Host "  â€¢ $f" -ForegroundColor Yellow
    }
    Write-Host ""
    Write-Host "ðŸ’¡ Deploy manually:" -ForegroundColor Cyan
    foreach ($f in $missing) {
        $localFile = $f -replace '/', '\'
...
if ($mismatches.Count -gt 0 -or $missing.Count -gt 0) {
    Write-Host "âŒ DEPLOYMENT VERIFICATION FAILED" -ForegroundColor Red
    exit 1
} else {
    Write-Host "âœ… ALL FILES DEPLOYED SUCCESSFULLY" -ForegroundColor Green
    exit 0
}
