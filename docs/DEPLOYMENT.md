...
1. ✅ Hostinger hosting account with WordPress plan
2. ✅ Git repository (GitHub/GitLab/Bitbucket)
3. ✅ WordPress installed on Hostinger (via hPanel one-click installer)
4. ✅ Domain configured and SSL certificate active
...
2. **Navigate to Git Deployment:**
   - Websites → Select your site → Advanced → Git
...
4. **Configure Deployment Settings:**
   ...
   - **Auto-deploy:** ✅ Enable (deploys automatically on push)
...
├── wp-content/                     # ✅ DEPLOYED to public_html/wp-content/
│   ├── themes/
│   │   ├── blocksy/                # Your active theme
│   │   └── twentytwenty*/          # Default themes
│   ├── plugins/
│   │   ├── your-custom-plugin/     # ✅ Custom plugins (tracked)
│   │   ├── woocommerce/            # ❌ Optional: exclude from git
│   │   └── elementor/              # ❌ Optional: exclude from git
│   ├── mu-plugins/                 # ✅ Must-use plugins (if any)
│   └── uploads/                    # ❌ Excluded (.gitignore)
│
├── .hostingerignore                # Deployment exclusions
├── .gitignore                      # Git tracking rules
│
├── infra/                          # ❌ Dev only (not deployed)
│   ├── dev/compose.yml             # Docker config
│   └── shared/db/                  # SQL initialization files
├── docs/                           # ❌ Dev only (not deployed)
├── config/                         # ❌ Dev only (not deployed)
└── tmp/                            # ❌ Dev only (not deployed)
```

**What Hostinger Deploys:**
- ✅ `wp-content/` directory (themes, plugins, mu-plugins)
- ❌ Everything else is excluded via [.hostingerignore](../.hostingerignore)
...
⚠️ **Hostinger Git integration does NOT automatically import SQL files** - this is a manual process.
...
1. **Access phpMyAdmin:**
   - Hostinger hPanel → Databases → Manage → phpMyAdmin
   - Or direct link: https://phpmyadmin.hostinger.com

2. **Select Database:**
   - Left sidebar → Click your WordPress database name
   - Usually: `u123456789_wordpress`

3. **Import SQL:**
   - Top menu → "Import" tab
   - Click "Choose File" → Select `initial-production-export.sql`
...
6. **Monitor deployment:**
   - Hostinger hPanel → Git → View deployment log
...
- ✅ `.gitignore` and `.hostingerignore`

**Optional (your choice):**
- 🟡 Third-party plugins (WooCommerce, Elementor, etc.)
...
- 🟡 Default themes (twentytwenty*, etc.)
  - **Recommendation:** Usually exclude, activate via WordPress admin instead

**Never track:**
- ❌ Uploads: `wp-content/uploads/` (too large, user-generated)
- ❌ WordPress core: `wp-admin/`, `wp-includes/` (Hostinger manages)
- ❌ Config files: `wp-config.php` (server-specific secrets)
- ❌ Dev infrastructure: `infra/`, `docs/`, `.github/`, `tmp/`, `config/`
...
# Download: public_html/wp-content/uploads/ → local/wp-content/uploads/
```
...
2. **Configure:**
   - LiteSpeed Cache → Dashboard → Click "Apply" on recommended settings
...
1. **Enable in hPanel:**
   - Websites → Select site → Performance → CDN
...
1. **Access PHP Configuration:**
   ...
   - Or: File Manager → Select PHP Options
...
1. **Enable SSL (if not already):**
   - hPanel → SSL → Select domain → Install SSL
...
1. **Enable Hostinger backups:**
   - hPanel → Backups → Configure schedule
...
1. Check deployment settings:
   - hPanel → Git → Verify auto-deploy is enabled
   - Verify branch is `main` (not `master`)

2. Check deployment logs:
   - hPanel → Git → View deployment history
   - Look for error messages

3. Manual deployment:
   - hPanel → Git → Click "Deploy Now"
...
2. Disable plugins:
   - File Manager → Rename `wp-content/plugins` to `wp-content/plugins-disabled`
   - Access site, re-enable plugins one by one

3. Increase PHP memory:
   - hPanel → PHP Configuration → `memory_limit = 256M`
...
1. Verify credentials in `wp-config.php`:
   ...
   - Compare with hPanel → Databases credentials

2. Check database exists:
   - hPanel → Databases → Verify database is listed
...
5. ✅ Monitor deployment logs after push
6. ✅ Keep development and production plugin versions in sync

### Database Management

1. ✅ Export database changes to SQL files in `infra/shared/db/`
2. ✅ Test SQL imports locally before applying to production
3. ✅ Backup production database before major schema changes
4. ✅ Use descriptive filenames: `251225-add-loyalty-tables.sql`
5. ✅ Document purpose in SQL file header comments

### Security

1. ✅ Never commit `wp-config.php` or database credentials
2. ✅ Keep `.hostingerignore` and `.gitignore` updated
3. ✅ Use strong passwords for WordPress admin accounts
4. ✅ Enable two-factor authentication
5. ✅ Regularly update plugins/themes (test in dev first)
6. ✅ Review Hostinger security scanner reports

### Performance

1. ✅ Enable LiteSpeed Cache
2. ✅ Optimize images before upload
3. ✅ Use Hostinger CDN for static assets
4. ✅ Minimize plugins (remove unused)
5. ✅ Monitor disk space and database size
...
### 🚧 Uploads Synchronization Strategy
