...
**⚠️ CRITICAL: Run this checklist BEFORE committing to develop or main**

```powershell
# 1. Check what files changed
Write-Host "`n📋 Deployment Verification Checklist`n" -ForegroundColor Cyan
...
Write-Host "`n=== Modified Tracked Files ===" -ForegroundColor Yellow  
git diff --name-only HEAD | Where-Object { 
    $_ -match '(wp-content/(mu-plugins|themes|plugins))' 
} | ForEach-Object { Write-Host "   ✏️  $_" }

Write-Host "`n=== Database/Page Content Changes ===" -ForegroundColor Magenta
Get-ChildItem restore\pages\ -Filter "*-*.html" | 
    Where-Object { $_.LastWriteTime -gt (Get-Date).AddHours(-24) } |
    ForEach-Object { Write-Host "   📝 Page backup: $($_.Name) (modified $($_.LastWriteTime))" }
```

**📋 Deployment Decision Matrix:**

| File Type | Auto-Deploy? | Action Required |
|-----------|--------------|-----------------|
| `wp-content/themes/**/*.js` | ✅ Yes | Add to git, push to main |
| `wp-content/themes/**/*.css` | ✅ Yes | Add to git, push to main |
| `wp-content/themes/**/*.php` | ✅ Yes | Add to git, push to main |
| `wp-content/mu-plugins/*.php` | ✅ Yes | Add to git, push to main |
| `wp-content/plugins/**/*` | ✅ Yes | Add to git, push to main |
| `restore/pages/*.html` | ❌ No | Manual: Use restore-page-*.php script |
| `restore/mu-plugins/*.php` | ❌ No | Manual: Copy to wp-content/mu-plugins/, then git add |
| `infra/shared/db/*.sql` | ❌ No | Manual: Apply via wp-cli or phpMyAdmin |
| Page content (database) | ❌ No | Manual: Export, document in release notes |

**🔍 Review Questions (Discuss with team/user):**

1. **New JavaScript files?**
   - Are they enqueued properly? (via wp_enqueue_scripts or MU plugin)
   - Version number set? (for cache busting)
   - Loaded on correct pages only?

2. **New MU Plugins?**
   - Do they auto-activate? (MU plugins always active)
   - Any dependencies on other plugins?
   - Tested in local environment?

3. **Modified existing files?**
   - Backward compatible changes?
   - Any breaking changes that need coordination?
   - Database migrations needed?

4. **Page content changes?**
   - Exported to restore/pages/?
   - Restore script created (restore-page-*.php)?
   - Documented in release notes?
   - IDs documented in deployment instructions?

5. **Assets (images, SVGs, fonts)?**
   - Added to git? (not gitignored)
   - File sizes reasonable? (<500KB per file)
   - Optimized? (compressed images, minified SVGs)
...
git add wp-content/themes/blocksy-child/assets/**/*.js
git add wp-content/themes/blocksy-child/assets/**/*.css

# Stage modified files
git add -u wp-content/

# Commit with descriptive message
git commit -m "feat: Add manager actions tabs + enhanced registration validation

- New: manager-actions-tabs.js (tab switching)
- New: td-manager-actions-assets.php (enqueue script)
- Enhanced: registration-form.js (retry logic, validation, spinner)
- Updated: td-registration-handler.php (nonce endpoint, debug logging)
- Page 21 & 36: Content changes (manual deployment required)

Deployment: 
- Code auto-deploys via Git
- Pages: Use restore-page-21.php & restore-page-36-fixed-js.php"
```

**📝 Update Release Notes:**

Add deployment instructions for manual steps:
...
```json
{
  "steps": [
    {
      "type": "deploy_code",
      "description": "Auto-deployed: JS, CSS, MU plugins"
    },
    {
      "type": "manual",
      "description": "Deploy Page 21 (Registration form)",
      "commands": [
        "podman cp restore/pages/register-profile-21.html wp:/var/www/html/tmp/",
        "podman exec wp wp eval-file /var/www/html/tmp/restore-page-21.php --allow-root"
      ]
    },
    {
      "type": "manual", 
      "description": "Deploy Page 36 (Manager Actions)",
      "commands": [
        "podman cp restore/pages/manager-actions-36.html wp:/var/www/html/tmp/",
        "podman exec wp wp eval-file /var/www/html/tmp/restore-page-36-fixed-js.php --allow-root"
      ]
    }
  ]
}
